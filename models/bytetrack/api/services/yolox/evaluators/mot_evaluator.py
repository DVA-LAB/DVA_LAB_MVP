...
class MOTEvaluator:
    ...
    def evaluate(self, model, distributed=False, half=False, trt_file=None, 
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(progress_bar(self.dataloader)):
            # imgs: torch.Size([1, 3, 800, 1440])
            # _: torch.Size([1, 1, 5])
            # info_imgs: [tensor([2160]), tensor([3840]), tensor([280]), ['DJI_0006_frame_00255.PNG']]
            # ids: tensor([[280]]) / torch.Size([1, 1])
            with torch.no_grad():
                # init tracker
                frame_id = info_imgs[2].item()
                # video_id = info_imgs[3].item()
                # img_file_name = info_imgs[4]
                img_file_name = info_imgs[3]
                ...
                if video_name not in video_names:
                    ...
                    video_names[cur_iter] = video_name
                if frame_id == 1:
                    tracker = BYTETracker(self.args)
                    if len(results) != 0:
                        # result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[cur_iter - 1]))
            ...
            if cur_iter == len(self.dataloader) - 1:
                ...
                result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[cur_iter]))
                write_results(result_filename, results)
