...
def read_image(path: str | Path, image_size: int | tuple[int, int] | None = None) -> np.ndarray:
    ...
    ##################################################################################################
    height, width = 2160, 3480
    slice_size = 840

    w_each = width // slice_size + 1
    h_each = height // slice_size + 1

    # 겹치는 정도 설정
    overlap_horizontal = ((((w_each)) * slice_size) - width) // (w_each - 1)
    overlap_vertical = ((((h_each)) * slice_size) - height) // (h_each - 1)

    print(overlap_horizontal)
    print(overlap_vertical)

    # 이미지를 slice 크기로 나누는 작업 수행
    slices = []
    x, y = 0, 0

    step_x = slice_size - overlap_horizontal 
    step_y = slice_size - overlap_vertical

    for h in range(h_each):
        for i in range(w_each):
            x1 = x
            y1 = y
            x2 = x1 + slice_size
            y2 = y1 + slice_size

            print(f'image[{y1}:{y2}, {x1}:{x2}],  ({x1},{y1})({x2},{y2}) ')
