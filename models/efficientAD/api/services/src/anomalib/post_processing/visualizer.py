...
class Visualizer:
    ...
    def visualize_batch(self, batch: dict) -> Iterator[np.ndarray]:
        ...
        for i in range(batch_size):
            ...
            image_result = ImageResult(
                ...
                save_path=batch["image_path"][i] if "image_path" in batch else None
            )
    ...
    def _visualize_full(self, image_result: ImageResult) -> np.ndarray:
        ...
        bbox = image_result.anomalous_boxes.tolist()   
        config = get_configurable_parameters('efficient_ad')
        filename = Path(image_result.save_path) 
        save_folder = os.path.join(config.visualization.image_save_path, filename.parent.name)
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)        
        
        pre_name = Path(filename.name).stem
        new_name = '.'.join([pre_name, 'json'])
