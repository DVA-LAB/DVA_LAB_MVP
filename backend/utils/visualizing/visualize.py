...
async def main(args):
    ...
    for image_path in image_paths:
        ...
        if rst:
            ...
            image = Image.fromarray(frame) # Original Image 
            draw = ImageDraw.Draw(image)
            bev_image = Image.fromarray(transformed_img) # BEV Image 
            bev_draw = ImageDraw.Draw(bev_image)
            gsd = gsd_bev
        ...
        if len(frame_bboxes) > 0 :
            for bbox_info in frame_bboxes:
                ...
                if class_id == 1:
                    bev_center_x, bev_center_y = bev_x2, bev_y2
                    center_x, center_y = x2, y2
                else:
                    bev_center_x, bev_center_y = (bev_x1 + bev_x2) / 2, (bev_y1 + bev_y2) / 2
                    center_x, center_y = (x1 + x2) / 2, (y1 + y2) / 2
                
                bev_center_x_dg, bev_center_y_dg = get_world_coordinate(bbox_for_dg, gsd, bev_center_x, bev_center_y)
                
                centers.append((center_x, center_y))
                bev_centers.append((bev_center_x, bev_center_y))
                centers_dg.append((bev_center_x_dg, bev_center_y_dg))
                classes.append(class_id)
                track_ids.append(track_id)

                if track_id == 999999:
                    dolphin_present = True
                    bev_merged_dolphin_center = (bev_center_x, bev_center_y)
                    merged_dolphin_center = (center_x, center_y)

                if class_id == 1:  # 선박인 경우
                    # 중심 좌표에 작은 초록색 점을 그림
                    outer_radius = 10  # 외부 원의 반지름
                    bev_draw.ellipse((bev_center_x - outer_radius, bev_center_y - outer_radius, bev_center_x + outer_radius, bev_center_y + outer_radius), outline=(0, 0, 255), width=10)
                ...
                if not (center_x == None and center_y == None):
                    ...
                    if dolphin_present:
                        draw_lines_and_distances(bev_draw, bev_centers, bev_merged_dolphin_center, classes, font, gsd)
                        draw_lines_and_distances(draw, centers, merged_dolphin_center, classes, font, gsd)
                        bev_nearest_distances = calculate_nearest_distance(dolphin_present, bev_merged_dolphin_center, bev_centers, classes, track_ids, gsd)
                        
                        for (track_id, cls), distance in bev_nearest_distances.items():
                            if cls == 1:
                                if distance <= 300:
        ...
        # 좌상단에 흰색 배경 사각형을 그립니다.
        dashboard_background = (0, 0, 700, 500)  # 좌표 (x0, y0, x1, y1)
        draw.rectangle(dashboard_background, fill=(255, 255, 255))
        bev_draw.rectangle(dashboard_background, fill=(255, 255, 255))
        
        for text, pos in zip(texts, text_positions):
            draw.text(pos, text, font=font, fill=font_color)
            bev_draw.text(pos, text, font=font, fill=font_color)
        ...
        if min(bev_img.shape[:2]) > 6000: 
            if bev_img.shape[0] > bev_img.shape[1] : # Height > Width 
                height = 6000
                aspect_ratio = float(height) / bev_img.shape[0]
                dsize = (int(bev_img.shape[1] * aspect_ratio), height)
            else : 
                width = 6000
                aspect_ratio = float(width) / bev_img.shape[1]
                dsize = (width, int(bev_img.shape[0] * aspect_ratio))
